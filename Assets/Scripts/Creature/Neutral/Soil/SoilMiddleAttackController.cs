...
public class SoilMiddleAttackController : FistBase
{
    ...
    public void Initialize(Vector2 startPosition, Vector2 endPosition, float radius, float _speed, NeutralObject _owner)
    {
        ...
        transform.rotation = Quaternion.Euler(new Vector3(0, 0, angle * 180 / Mathf.PI));  // Here rotation need degree, not radian
        // Set Target Position
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (collision.transform.GetComponent<NeutralObject>() != owner)
            {
